...
cluster through mechanisms rather than because it has an essence
\citep{boyd1991,boyd1999}. The word \term{homeostatic} has tended to carry more
...
feedback-sensitive control. This paper treats projectibility as the target of
kind claims and offers a diagnostic for distinguishing the ways it can be
secured. A stable property cluster requires reliable co-occurrence together with
a specified projection. A networked kind requires internal structure among
central, derivative, and evidentially useful properties. A control-secured kind
requires the more demanding pattern of perturbation-sensitive correction that
preserves a higher-scale relation through lower-scale variation. Since the
maintenance literature already separates the stabilizers that keep a cluster
clustered from the narrower case of a controller, and since Weinberger's account
of homeostasis as causal control makes the stronger standard precise
\citep{weinberger2026HomeostasisCausalControl}, the distinction can be drawn from
within Boyd's framework rather than against it. The Watt governor serves as a
calibration case, glycemic regulation as a harder biological extension, English
interjections as a negative linguistic case, and agreement monitoring as a
candidate positive one. The result is a disciplined way of using Boyd's insight that keeps
\term{homeostatic} from becoming a loose synonym for stable, mechanism-maintained,
or projectible.
\end{abstract}
...
projectible profiles in ways that deserve separate treatment.
...
Glycemic regulation is instructive because it is harder than the governor. The
relevant candidate is not glucose as a substance, nor a single blood value, but the
...
display corrective control leaves the others untouched, since they remain real and projectible, while
showing the analyst what to look for, and what would count as having found it,
before the strongest description is reached for. What philosophers of science gain is a domain-neutral test that
distinguishes projectible stability, ordered dependence, and corrective control, and
that says what evidence would move a case from one to another.
